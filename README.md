...
这个项目的定位不是通用聊天机器人，而是一个面向食品质量/法规人员的合规初筛工具原型：先证明“问题能检索到依据、风险判断可复现、无依据能拒答”，再逐步扩展到 RAG + LLM + 企业审版流程。

![食安智问演示预览](assets/demo-preview.svg)

## 适合展示给谁

- AI 应用 / AI 解决方案岗位：展示从业务问题到可运行 AI 产品原型的落地能力。
- FDE / 解决方案工程岗位：展示需求拆解、数据结构设计、Demo 交付和边界说明。
- 食品企业 / 国企数字化岗位：展示食品专业知识如何转成合规审版、法规咨询和流程数字化工具。
...
| 能力 | 当前实现 |
|---|---|
| 法规语料 | 30 条结构化条款，覆盖食品安全法、GB 7718、GB 28050、GB 2760、食品标识监管要求等演示场景。 |
| 问答闭环 | 问题输入、条款检索、风险分级、整改建议、引用依据、免责声明。 |
| 拒答机制 | 对知识库外问题返回“待人工核验”，不生成无依据结论。 |
| Web 演示 | 静态 HTML/CSS/JS，可直接打开或部署到 GitHub Pages。 |
| CLI 演示 | Python 标准库实现，无第三方依赖。 |
| 测试集 | 30 条黄金问题 + 3 个核心回归样例。 |
...
| 问题 | 取舍 |
|---|---|
| 为什么不用 Streamlit | 静态 Web 更适合面试和作品集展示，不会因为免费托管休眠导致现场打不开。 |
| 为什么不用向量库 | 当前语料规模小，关键词、主题和法规标题检索更可解释，也更方便验证。 |
| 为什么不用 LLM 直接判断 | 合规结论必须来自条款和规则，LLM 后续只作为表达层。 |
| 为什么保留 CLI | CLI 方便测试、验收和后续接 FastAPI，不被前端展示层绑定。 |
...
完整讲稿见 [docs/interview-notes.md](docs/interview-notes.md)。核心表达：

> 我没有先做通用 RAG 套壳，而是把场景收窄到预包装食品标签合规。因为合规问答最关键的是证据、版本、适用范围和拒答能力，所以 MVP 先用结构化法规库、规则风险判断和强制引用完成闭环，再预留 LLM 接口。
